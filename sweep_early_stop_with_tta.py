...
HYPOTHESIS: Since TTA boosts accuracy by ~0.8% (93.2% → 94.01%), we might be able
to train for fewer epochs and still hit 94% when evaluated with TTA.

APPROACH:
- Test epoch values: 9.0, 9.1, 9.2, 9.3, 9.4, 9.5, 9.6, 9.7, 9.8, 9.9
- For each epoch value, train N models
- Evaluate with TTA (using optimal weight if available, else 0.50)
- Track: training time, TTA accuracy, no-TTA accuracy
- Find: minimum epochs needed to achieve ≥94% with TTA

OBJECTIVE: Reduce training time while maintaining ≥94% accuracy with TTA
"""
...
def train_model_to_epoch(target_epochs, run_id, tta_weight=0.50, verbose=False):
    """
    Train a single model for exactly target_epochs.
    ...
    # Learning rate schedule (same as baseline)
...
def main():
    parser = argparse.ArgumentParser(
        description='Early Stopping with TTA Compensation Experiment'
    )
    ...
    parser.add_argument('--tta_weight', type=float, default=0.50,
                        help='TTA untrans_weight to use (default: 0.50, use value from exp410 if known)')
    parser.add_argument('--experiment_name', type=str, default='exp420-early-stop-tta',
    ...
    print("\n" + "="*100)
    print("EXPERIMENT 420: EARLY STOPPING WITH TTA COMPENSATION")
    print("="*100)
    print(f"Hypothesis: TTA boost allows training for fewer epochs while maintaining ≥94% accuracy")
    print(f"\nConfiguration:")
    ...
    for epoch_val in epoch_values:
        print(f"\n{'='*100}")
        print(f"Testing {epoch_val} epochs ({args.runs_per_epoch} runs)")
        print(f"{'='*100}")

        # Initialize logger for this epoch value
        logger = ExperimentLogger(
            experiment_name=f"{args.experiment_name}_epoch{int(epoch_val*10):02d}",
            experiment_description=f"Early stopping experiment: {epoch_val} epochs with TTA",
            hyperparameters={
                **hyp,
        ...
        for run_id in range(args.runs_per_epoch):
            model, train_time, final_train_acc, val_acc, tta_val_acc = train_model_to_epoch(
                epoch_val, run_id, tta_weight=args.tta_weight, verbose=(run_id < 3)
            )
    ...
    # Final comparison
    print("\n" + "="*100)
    print("RESULTS SUMMARY - EPOCH OPTIMIZATION")
    print("="*100)
    ...
    if not valid_epochs:
        ...
        print(f"\n1. MINIMUM EPOCHS FOR ≥94% ACCURACY (with TTA):")
        ...
        # Find fastest that meets target
        fastest = min(valid_epochs, key=lambda x: x[1]['mean_time'])

        if fastest[0] != min_epoch[0]:
            print(f"\n2. FASTEST TIME WITH ≥94% ACCURACY:")
            print(f"   Epochs: {fastest[0]:.1f}")
            print(f"   Mean Time: {fastest[1]['mean_time']:.2f}s")
            print(f"   TTA Accuracy: {fastest[1]['mean_tta']:.4f}")

    # Export CSV
    csv_path = Path('experiments') / f'{args.experiment_name}_comparison.csv'
    csv_path.parent.mkdir(exist_ok=True)
    with open(csv_path, 'w') as f:
        f.write("epochs,mean_no_tta_acc,mean_tta_acc,mean_time,success_rate,achieves_94,time_saved_vs_baseline\n")
        for epoch_val in sorted(all_results.keys()):
            r = all_results[epoch_val]
            ...
            f.write(f"{epoch_val:.1f},{r['mean_no_tta']:.6f},{r['mean_tta']:.6f},"
                   f"{r['mean_time']:.4f},{r['success_rate']:.4f},{achieves},{time_saved:.4f}\n")
